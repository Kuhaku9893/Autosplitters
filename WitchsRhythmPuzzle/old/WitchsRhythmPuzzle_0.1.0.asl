...
startup
{
    ...
    settings.Add("stageSelect", false, "Stage Select for Nonstop Mode");
    settings.Add("stageSelect1", false, "Set Entail Stage", "stageSelect");
    settings.Add("stageSelect11", false, "Set Ibis Stage", "stageSelect");
}
...
update
{
    ...
    // stage select
    if (settings["stageSelect"] && current.nonstop == 1 && current.SceneName == "event")
    {
        int stage = current.stage;
        if (settings["stageSelect1"])
            stage = 1;
        if (settings["stageSelect11"])
            stage = 11;
        
        print("set stage : " + stage.ToString());
        IntPtr p;
        new DeepPointer("UnityPlayer.dll", 0x014E0D64, 0x2c8, 0x368, 0x74, 0x84, 0x4, 0x38, 0x20).DerefOffsets(game, out p);
        try
        {
            game.WriteBytes((IntPtr)p, BitConverter.GetBytes((int)stage));
        }
        catch
        {
