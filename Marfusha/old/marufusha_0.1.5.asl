...
state("Marufusha", "ver1.0.0.5")
{
    ...
    float bossHp : "UnityPlayer.dll", 0x01781C20, 0xf0, 0x0, 0x30, 0x30, 0x68, 0x28, 0x78;
    float bossOriginalHp : "UnityPlayer.dll", 0x01781C20, 0xf0, 0x0, 0x30, 0x30, 0x68, 0x28, 0x7c;
}
...
update
{
    ...
    using(Microsoft.Win32.RegistryKey regKey = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(@"Software\hinyari9\Marufusha"))
    {
        ...
        foreach (string name in regKey.GetValueNames())
        {
            ...
            if (name.StartsWith("equipedGunName_"))
            {
                ...
                // print("GunName[] : " + gunNameByte[0] + gunNameByte[1]);
                // string ss = System.Text.Encoding.UTF8.GetString(gunNameByte);
                // print("GunName \t: " + ss);
            }
        } // end foreach
    ...
    {
        string limit = wpLimit > 0 ? wpLimit.ToString() : "-" ;
        // if (settings["challenge"])
            // today = vars.endlCurrentDay;
        
        string wpName = "-";
        if (gunNameByte != null)
            wpName = System.Text.Encoding.UTF8.GetString(gunNameByte);

        vars.tcss[1].Text1 = wpName; // Text1は左寄せの文字
        vars.tcss[1].Text2 = "Limit " + limit; // Text2は右寄せの文字
    }
}
