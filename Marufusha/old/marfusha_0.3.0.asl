...
// ゲームver1.1.0に対応
// ゲームver1.0.0.6以前との互換性はなし
// チャレンジモードは一次的に非対応としておく
...
state("Marfusha", "ver1.1.0")
{
    ...
    string4 weaponName : "mono-2.0-bdwgc.dll", 0x00495A68, 0xc08, 0x78, 0x60, 0x18, 0x14;
}
...
update
{
    ...
    if (settings["main"])
    {
        ...
        // for reset
        if ((current.basicSalary < old.basicSalary) && (timer.CurrentPhase == TimerPhase.Running))
        {
            vars.needReset = true;
            return true;
        }
